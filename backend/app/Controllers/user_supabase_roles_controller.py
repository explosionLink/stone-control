# app/Controllers/user_supabase_roles_controller.py
...
from app.Services.role_service import RoleService               # Service per la logica di business sui ruoli e user_supabase_roles
from app.Schemas.user_supabase_role import AssignRoleInput               # Schema Pydantic per input di assegnazione ruolo
from app.Schemas.role import RoleRead                           # Schema Pydantic di output per i ruoli


class UserSupabaseRolesController:
    def __init__(self): ...                                     # Nessuna inizializzazione speciale

    async def list_user_supabase_roles(
        self,
        ...
        roles = await svc.user_supabase_roles.list_user_supabase_roles(user_id)   # Deve restituire una lista di oggetti Role (join sul ponte)
    ...
    async def assign_role(
        ...
        await svc.user_supabase_roles.assign(payload.user_id, payload.role_id)  # Crea la riga nella tabella ponte
    ...
    async def unassign_role(
        ...
        svc = RoleService(db)                                   # Istanzia il service
        ok = await svc.user_supabase_roles.unassign(user_id, role_id)    # Prova a rimuovere l’associazione
