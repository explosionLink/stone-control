...
from app.Services.user_supabase_service import UserSupabaseService               # Service che incapsula la business logic sugli utenti
from app.Schemas.user_supabase import (                             # Schemi Pydantic: input/output
    UserSupabaseCreate,
    UserSupabaseUpdate,
    UserSupabaseRead,
)
...
class UsersController:
    ...
    async def list_users(
        ...
        return [UserSupabaseRead.model_validate(r) for r in rows]   # Valida/serializza con Pydantic (DTO di output)
    ...
    async def get_user(
        ...
        return UserSupabaseRead.model_validate(row)                 # Serializza il record a schema di lettura
    ...
    async def create_user(
        ...
        return UserSupabaseRead.model_validate(row)                 # Serializza il risultato
    ...
    async def update_user(
        ...
        if not row:                                             # Se l’utente non esiste
            raise HTTPException(status_code=404, detail="User non trovato")  # 404 Not Found
        return UserSupabaseRead.model_validate(row)                 # Serializza l’utente aggiornato
